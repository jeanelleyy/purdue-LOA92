  ...
  useEffect(()=>{    
    if(monitoringDone && countingDone){
      ...
      if(props.trials == 0){
        ...
        let monitoring_entry = writeM(props.levelType, props.trials+1, mResult.duration, mResult.duration < 15000, imgPath, mResult.count >=0 ? mResult.count : -1, mResult.wrong);
        props.setMData([...props.mdata, patternTag, monitoring_entry]);
      }
      else{
        if(!timerOk && !(mResult >=0)){
          let monitoring_entry = writeM(props.levelType, props.trials+1, 15000, false, imgPath, -1, "No Screening");
      ...
      let counting_entry = writeC(props.levelType, props.trials+1, cResult.duration, imgPath, cResult.choice, rand_bag, cResult.count, props.startData[myIndex]);
